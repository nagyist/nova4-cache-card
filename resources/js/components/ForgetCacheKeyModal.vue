<template>
    <modal @modal-close="handleClose">
        ...
        >
            <div class="p-8">
                <heading :level="2" class="mb-6">{{__('Forget Cache Key')}}</heading>
                <p class="text-80">{{__('Provide the cache key you would like to forget?')}}</p>
            ...
            <div class="bg-30 px-6 py-3 flex">
                <div class="ml-auto">
                  ...
                  <button dusk="confirm-get-key-button" ref="confirmButton" data-testid="confirm-button" @click.prevent="handleConfirm" class="rounded btn btn-default btn-primary p-2 bg-primary-500 text-white">{{__('Forget')}}</button>
                </div>
            </div>
        </div>
    </Modal>
